...
class WarningEvaluator {
  ...
  evaluateWarnings(Carparams newParams) {
    if (newParams.batteryStateOfCharge < 15 && (currentParams.batteryStateOfCharge >= 15)) {
      final warning = Warning(Warning.STATE_RED, 'Batterieladung < 15%', 'battery');
      warningController.add(warning);
    } else if (newParams.batteryStateOfCharge < 30 && (currentParams.batteryStateOfCharge >= 30)) {
      final warning = Warning(Warning.STATE_YELLOW, 'Batterieladung < 30%', 'battery');
      warningController.add(warning);
    }
    if (newParams.calculatedRemainingDistance < 15 && (currentParams.calculatedRemainingDistance >= 15)) {
      final warning = Warning(Warning.STATE_RED, 'Reichweite < 15 km', 'battery');
      warningController.add(warning);
    } else if (newParams.calculatedRemainingDistance < 30 && (currentParams.calculatedRemainingDistance >= 30)) {
      final warning = Warning(Warning.STATE_YELLOW, 'Reichweite < 30 km', 'battery');
      warningController.add(warning);
    }
    if (newParams.wipingWaterLevel < 15 && (currentParams.wipingWaterLevel >= 15)) {
      final warning = Warning(Warning.STATE_RED, 'Wischwasser < 15%', 'battery');
      warningController.add(warning);
    } else if (newParams.wipingWaterLevel < 30 && (currentParams.wipingWaterLevel >= 30)) {
      final warning = Warning(Warning.STATE_YELLOW, 'Wischwasser < 30%', 'battery');
      warningController.add(warning);
    }
    if (newParams.powerConsumption > 35 && (currentParams.powerConsumption <= 35)) {
      final warning = Warning(Warning.STATE_RED, 'Stromverbrauch hoch', 'battery');
      warningController.add(warning);
    }
    if (newParams.tirePressureBackLeft < 2 && (currentParams.tirePressureBackLeft >= 2)) {
      final warning = Warning(Warning.STATE_RED, 'Reifendruck hinten links sehr niedrig', 'battery');
      warningController.add(warning);
    } else if (newParams.tirePressureBackLeft < 2.5 && (currentParams.tirePressureBackLeft >= 2.5)) {
      final warning = Warning(Warning.STATE_YELLOW, 'Reifendruck hinten links niedrig', 'battery');
      warningController.add(warning);
    }
    if (newParams.tirePressureBackRight < 2 && (currentParams.tirePressureBackRight >= 2)) {
      final warning = Warning(Warning.STATE_RED, 'Reifendruck hinten rechts sehr niedrig', 'battery');
      warningController.add(warning);
    } else if (newParams.tirePressureBackRight < 2.5 && (currentParams.tirePressureBackRight >= 2.5)) {
      final warning = Warning(Warning.STATE_YELLOW, 'Reifendruck hinten rechts niedrig', 'battery');
      warningController.add(warning);
    }
    if (newParams.tirePressureFrontLeft < 2 && (currentParams.tirePressureFrontLeft >= 2)) {
      final warning = Warning(Warning.STATE_RED, 'Reifendruck vorne links sehr niedrig', 'battery');
      warningController.add(warning);
    } else if (newParams.tirePressureFrontLeft < 2.5 && (currentParams.tirePressureFrontLeft >= 2.5)) {
      final warning = Warning(Warning.STATE_YELLOW, 'Reifendruck vorne links niedrig', 'battery');
      warningController.add(warning);
    }
    if (newParams.tirePressureFrontRight < 2 && (currentParams.tirePressureFrontRight >= 2)) {
      final warning = Warning(Warning.STATE_RED, 'Reifendruck vorne rechts sehr niedrig', 'battery');
      warningController.add(warning);
    } else if (newParams.tirePressureFrontRight < 2.5 && (currentParams.tirePressureFrontRight >= 2.5)) {
